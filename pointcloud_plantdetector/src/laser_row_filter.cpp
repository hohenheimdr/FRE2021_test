 ...
 * 
 * dr 	07.06.17	changed some details about the cluster algorithm changed it so that the biggest cluster gets choosen
 * 
 * 
 * 
 *  * */
...
// public variables
	...
	{
		//changed that the biggest cluster gets returned
		if(conf.use_eucledian_cluster)
		{
		 ...
		 // Creating the KdTree object for the search method of the extraction
		  ...
		  int j = 0;
		  //now move trough all clusters of the point cloud... and search for the biggest cluster
		  for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin (); it != cluster_indices.end (); ++it)
		  {
			...
			//write the biggest cluster to the output
			if(cloud_cluster->points.size()>output->points.size())
