...
public:
	...
	bool closed_loop,toggle_ch1,toggle_ch2;
	//PID values
	double P,Pang;
	double I,Iang;
	double D,Dang;
	...
	void setSpeed(const geometry_msgs::TwistStamped::ConstPtr& msg)
	{
		...
		//std::cout<<"actual_time:"<<ros::Time::now()<<std::endl;
		//std::cout<<"cmd_time:"<<msg->header.stamp<<std::endl;
		ros::Duration time_dif=(ros::Time::now()-msg->header.stamp);
		std::cout<<"time_dif:"<<time_dif<<" deadzone:"<<deadzone<<std::endl;
	
	if(time_dif.toSec()>=deadzone)
		{
		speed_soll=0.0;
		angle_soll=0.0;
	...
	//check if cmd vel was 0
	 ...
	 if(toggle_ch1)
		speedcommand_ch1=-speedcommand_ch1;
	 
	 if(toggle_ch2)
		speedcommand_ch2=-speedcommand_ch2;
...
int main( int argc, char** argv )
{
	...
	n.param<double>("deadzone",mc.deadzone,0.05);
	n.param<std::string>("port",mc.port,"/dev/ttyACM0"); 
	n.param<bool>("toggle_ch1",mc.toggle_ch1,false);
	n.param<bool>("toggle_ch2",mc.toggle_ch2,false);
    ...
    n.param<double>("output_offset_left",mc.output_offset_right,0);
     n.param<double>("output_offset_right",mc.output_offset_left,0);
